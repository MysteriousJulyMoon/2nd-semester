...
namespace mt
{
	...
	public:
		...
		bool Setup(float x, float y)
		{
			...
			// Загрузка текстуры корабля(фона)
			if (!m_textureShip.loadFromFile("assets\\6lab.png"))
			{
				std::cout << "Error while loading 6lab.png" << std::endl;
				return false;
			}
			m_spriteShip.setTexture(m_textureShip);

			m_spriteShip.setScale(0.3f, 0.3f);
